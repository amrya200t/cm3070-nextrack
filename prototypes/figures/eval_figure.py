...
SERIES_COLORS = {"Popularity baseline": "#1baf7a", "ALS session-vector": "#2a78d6"}
TEXT_PRIMARY = "#0b0b0b"
...
def main() -> None:
    ...
    pop = [results["popularity"][m] for m in ("recall", "ndcg", "mrr")]
    als = [results["als_session_vector"][m] for m in ("recall", "ndcg", "mrr")]

    fig, ax = plt.subplots(figsize=(7, 4), dpi=200, facecolor=SURFACE)
    ...
    bars = [
        ax.bar(
            [i - width / 2 - gap / 2 for i in x], pop, width,
            label="Popularity baseline", color=SERIES_COLORS["Popularity baseline"],
        ),
        ax.bar(
            [i + width / 2 + gap / 2 for i in x], als, width,
            label="ALS session-vector", color=SERIES_COLORS["ALS session-vector"],
        ),
    ]
    for group in bars:
        for rect in group:
